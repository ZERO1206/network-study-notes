...
浏览器请求 `GET /../../../etc/passwd HTTP/1.1`，程序直接映射到 `www/../../../etc/passwd` → 读取任意系统文件。

**改进方向：** 检查 `..` 并拒绝，或 `realpath()` 归一化后验证路径是否在 `www/` 目录下。

### 2. 只解析了第一行

`Host`、`User-Agent`、`Connection` 等头部全部忽略。

**改进方向：** 逐行解析头部，支持 `Connection: keep-alive`（复用连接）、`Content-Type: ...`（POST 请求体）。

### 3. 不支持 POST

只读了请求行，POST 请求的正文（表单数据、上传文件）没读。

**改进方向：** 解析 `Content-Length` 头部，继续 recv 读取请求体。

### 4. 大文件阻塞 select

`fread` 把整个文件读进内存再 send。大文件（几百 MB）会让这个客户端独占处理时间，其他客户端被堵死。

**改进方向：** 分块读取 + 分块发送，或使用 `sendfile()` 系统调用零拷贝传输。

### 5. 无并发保护

select 在单线程内串行处理。每个 `send` 和 `fread` 期间其他请求在排队。

**改进方向：** epoll + 线程池，或事件驱动架构。
