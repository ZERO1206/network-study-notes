...
| | 请求行 | Host 头 | 说明 |
|------|--------|---------|------|
| 普通请求（浏览器→百度） | `GET / HTTP/1.1` | `Host: www.baidu.com` | 路径是相对路径，Host 仅用于虚拟主机区分 |
| 代理请求（浏览器→代理） | `GET http://www.baidu.com/ HTTP/1.1` | `Host: www.baidu.com` | 路径是**完整 URL**，代理从这里解析目标主机 |

**代理收到的请求行里直接带了完整的 `http://www.baidu.com/`**，这是浏览器知道自己在用代理时自动切换的格式。代理解析出目标主机 → DNS 获取 IP → connect 目标 → 转发请求。
